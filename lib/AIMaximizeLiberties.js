...
class AIMaximizeLiberties extends AI {
     ...
     * Infers the color of the token based on the last move.
    ...
    move(board, lastMove){
        ...
        newMove.color = this.getNextMoveColor(lastMove);
        ...
        if (lastMove.pass) {
            ...
            if (maxFreeLibs == -1) {
                ...
                var indexes = this._pickRandomMove(liberties[maxFreeLibs]);
                newMove.x = indexes[0];
                newMove.y = indexes[1];
            }
        }
