...
class AIFormEyes extends AI {
    ...
    _identifyUniqueLiberties(armies) {
        // console.log(util.inspect(armies, {showHidden: false, depth: null}));

        const liberties = [];
        for(let army of armies) {
            for(let lib of army.liberties) {
                // console.log(liberties.some((elem, i, arr) => {return elem[0] == lib[0] && elem[1] == lib[1]}));
                if (!liberties.some((elem, i, arr) => {return elem[0] == lib[0] && elem[1] == lib[1]}))
                    liberties.push(lib);
                // if (lib not in liberties.push(lib);
            }
        }
